...
| Any6_FitnessTracker | `com.roneaglin.any6_fitnesstracker` | 1.1 / 5 | **net10.0 / API 36** | 21 | `C:\keystores\any6alias.playupload.keystore` (alias `any6alias`, SHA-256 `18:6D:6E:22…`) — **not** `eaglin.any6_fitness.keystore` / repo `key.keystore` (different key; Play rejects it) | **1.1 (5) AAB built + signature-verified 2026-08-29** (`release\com.roneaglin.any6_fitnesstracker-v1.1-vc5.aab`, commit `51e7396`): adds 7-day share window w/ Play link + uncapped activity list, backups saved to Downloads w/ in-app restore picker, no restart on restore (all device-tested 2026-08-29 on the release-signed build). Supersedes vc4/vc3. Production-access request still in review — when granted, Ron uploads **1.1 (5)**; then tag `v1.1`. ⚠ Deploy lesson 2026-08-29: a Debug `-t:Run` deploy over the release-signed install wiped the app's data (signature mismatch ⇒ uninstall); recovered via Google Auto Backup minus one day. Device-test via release-signed `adb install -r` only |
...
| PunchMonkey | `com.eaglin.punchmonkey` | 1.22 / 23 | **net10.0 / API 36** | 26 | `C:\keystores\mycompany.myapp.keystore` (alias `myapp`) — legacy name, **do not rename**: it is the registered upload key | **1.22 (23) API-36 rebuild built + signature verified 2026-08-23** (commit `6d236d0`, feature-identical to vc22) — device smoke test pending, then upload before Nov 1; Play Console showed vc22 in Production (Aug 18) — confirm its rollout, tag `v1.22`, bump server `LatestAppVersion` to 1.22 + deploy. Next feature release = Course/Series re-presentation (`PunchMonkeyServer\COURSE_ARCHITECTURE.md`); server-side "My Courses" library page (captures the app's Design Course uploads) deployed 2026-08-27 (PunchMonkeyServer `91cbe4c`) |
...
- [x] Any6_FitnessTracker — retargeted (`d6271ee`, 2026-07-28); **1.1 (vc5) API-36 AAB built 2026-08-29** (signing auto via `Directory.Build.props`) — upload once production access is granted, before Nov 1.
...
- [x] PunchMonkey — retargeted (`91ac48d`); **versionCode-23 API-36 build DONE + signature verified 2026-08-23** — smoke test, then upload before Nov 1.
