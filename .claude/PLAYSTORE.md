...
| Any6_FitnessTracker | `com.roneaglin.any6_fitnesstracker` | 1.1 / 4 | **net10.0 / API 36** | 21 | `C:\keystores\any6alias.playupload.keystore` (alias `any6alias`, SHA-256 `18:6D:6E:22…`) — **not** `eaglin.any6_fitness.keystore` / repo `key.keystore` (different key; Play rejects it) | **1.1 (4) AAB BUILT + signature-verified 2026-08-27** (`release\com.roneaglin.any6_fitnesstracker-v1.1-vc4.aab`): points per exercise, multi-goal picker, By Activity breakdown (`aa4e2d3`/`55d9f48`/`2383c28`) + goal-met share w/ Play link (`fdb40a8`, device-tested 2026-08-27); signing now auto via gitignored `Directory.Build.props` (`a8b400b`); production-access request still in review — when granted, Ron uploads **1.1 (4)**, NOT the superseded 1.0 vc3 API-35 file; then tag `v1.1` |
...
- [x] Any6_FitnessTracker — retargeted (`d6271ee`, 2026-07-28); 1.1 (vc4) API-36 release prepped 2026-08-22 (build = `publish-release.ps1`, needs Ron's password) — upload once production access is granted, before Nov 1.
