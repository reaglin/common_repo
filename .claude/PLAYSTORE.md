...
| Any6_FitnessTracker | `com.roneaglin.any6_fitnesstracker` | 1.1 / 4 | **net10.0 / API 36** | 21 | `C:\keystores\any6alias.playupload.keystore` (alias `any6alias`, SHA-256 `18:6D:6E:22…`) — **not** `eaglin.any6_fitness.keystore` / repo `key.keystore` (different key; Play rejects it) | **1.1 (4) ready to build**: points per exercise, multi-goal picker, By Activity breakdown (commits `aa4e2d3`/`55d9f48`/`2383c28`, device-tested 2026-08-22) + goal-met share w/ Play link (`fdb40a8`, on test phone 2026-08-27 — **test share, then rebuild the vc4 AAB** via `publish-release.ps1` so it's included); production-access request still in review as of 2026-08-22 — when granted, Ron uploads **1.1 (4)**, NOT the superseded 1.0 vc3 API-35 file; then tag `v1.1` |
